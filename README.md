...
1. **`start.command` 재실행**
   - start.command는 자동으로 3회까지 재시도합니다
   - 대부분의 경우 2-3회 시도에서 성공
...
2. **USB 재연결** (가장 확실)
   ...
   2. 5초 대기
   3. USB 케이블 재연결
   4. start.command 다시 실행
   ```

3. **카메라 USB 모드 변경** (근본 해결)
   ```
   카메라 메뉴 → 설정 → USB 연결
   PTP → Mass Storage 또는 PC Remote
   ```

**⚠️ 참고:** macOS 시스템이 카메라를 자동으로 점유하기 때문에 연결이 실패할 수 있습니다. 상세한 기술 분석은 `TROUBLESHOOTING_ANALYSIS.md` 참조.
